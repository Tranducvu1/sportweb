...
const ListProduct = () => {
  ...
  useEffect(() => {
    ...
    if (selectedFilters.sizes.length > 0) {
      result = result.filter(product =>
        product.bienthes.some(bt => selectedFilters.sizes.includes(bt.size))
      );
    }
    ...
    if (selectedFilters.priceRanges.length > 0) {
      result = result.filter(product => {
        const prices = product.bienthes.map(bt => bt.price);
        return selectedFilters.priceRanges.some(range => {
          const [min, max] = range;
          //check if any price in the product is in the range
  ...
  const calculateFinalPrice = (product) => {
    const minPrice = Math.min(...product.bienthes.map(bt => bt.price));
    const discount = product.discountPercent || 0;
    return minPrice * (1 - discount / 100);
  };
